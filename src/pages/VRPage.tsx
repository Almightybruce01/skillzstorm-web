  ...
  return (
    <div className="pt-20 min-h-[100vh] w-full max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pb-32">
      ...
      <section className="text-center py-16 animate-slide-up relative">
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[400px] h-[250px] bg-gradient-to-r from-[#00e6e6]/15 via-[#0099ff]/10 to-[#9933ff]/15 blur-[80px] rounded-full color-shift-slow pointer-events-none" />
        <div className="relative inline-block mb-4">
          <div className="absolute inset-0 blur-[50px] bg-[#00e6e6]/25 rounded-full scale-[2.5] animate-pulse-slow" />
        ...
        <p className="text-[#00e6e6] font-black tracking-[0.5em] text-xs sm:text-sm mb-5 neon-glow-cyan animate-slide-up delay-200">
          LEARN IN VIRTUAL REALITY
        </p>
        <p className="text-white/40 max-w-xl mx-auto text-base leading-relaxed animate-slide-up delay-300">
          Step inside your lessons. Experience math, science, history, and more
          in immersive 3D worlds. <span className="text-white/60 font-semibold">Multiple ways to play.</span>
        </p>
      </section>
      ...
      <section className="mb-20">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          ...
          <VRMethodCard
            emoji="📱" title="Phone-in-Headset" subtitle="EASIEST — $29.99" color="#0099ff"
          ...
          {/* Method 2: Web VR */}
          <VRMethodCard
            emoji="🌐" title="Web VR (WebXR)" subtitle="FREE — No Download" color="#00ff80"
          ...
          {/* Method 3: Standalone */}
          <VRMethodCard
            emoji="🎧" title="StormVR Pro Standalone" subtitle="ALL-IN-ONE — $149.99" color="#9933ff"
          ...
          <VRMethodCard
            emoji="👁️" title="Apple Vision Pro" subtitle="SPATIAL COMPUTING" color="#ff8000"
      ...
      <section className="mb-20">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-5">
          {[
            { emoji: '🧮', name: 'VR Math Arena', desc: 'Solve equations floating in 3D space', color: '#0099ff' },
            { emoji: '🔬', name: 'VR Science Lab', desc: 'Conduct virtual experiments', color: '#00ff80' },
            { emoji: '🌍', name: 'VR Geo Explorer', desc: 'Travel the globe in VR', color: '#9933ff' },
            { emoji: '📐', name: 'Geometry Runner 3D', desc: 'Run through geometric worlds', color: '#ff8000' },
            { emoji: '⚗️', name: 'VR Chemistry', desc: 'Mix elements in 3D lab', color: '#ff3399' },
            { emoji: '🏺', name: 'VR History Walk', desc: 'Walk through ancient civilizations', color: '#ffe600' },
            { emoji: '🛡️', name: 'Storm Defenders VR', desc: '3D tower defense with learning', color: '#ff2626' },
            ...
            >
              ...
              <h3 className="font-bold text-sm mb-1 text-white group-hover:transition-colors duration-300"
                onMouseEnter={(e) => { (e.target as HTMLElement).style.color = game.color; }}
                onMouseLeave={(e) => { (e.target as HTMLElement).style.color = 'white'; }}
              >{game.name}</h3>
              <p className="text-white/25 text-xs group-hover:text-white/45 transition-colors">{game.desc}</p>
            </div>
          ))}
        </div>
      </section>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-white/[0.06] bg-white/[0.02]">
                <th className="py-4 px-4 text-white/40 text-xs font-black tracking-wider">FEATURE</th>
                <th className="py-4 px-4 text-[#0099ff] text-xs font-black tracking-wider">LITE</th>
                <th className="py-4 px-4 text-[#9933ff] text-xs font-black tracking-wider">PRO</th>
                <th className="py-4 px-4 text-[#00ff80] text-xs font-black tracking-wider">WEB</th>
              </tr>
            </thead>
      ...
      <section className="mb-20">
        ...
        <div className="game-card !p-8" style={{ background: 'linear-gradient(135deg, rgba(0,255,128,0.04), rgba(0,153,255,0.03))' }}>
          <p className="text-white/50 text-sm mb-6">Already have a VR headset? Here's the fastest way to try SkillzStorm in VR:</p>
          ...
          <p className="text-center text-white/25 text-xs mt-6">Works on Meta Quest 2/3/Pro • Pico 4 • HTC Vive Focus • Any WebXR headset</p>
        </div>
      </section>
          ...
          {[
            ...
            <details key={i} className="game-card !p-5 cursor-pointer group animate-slide-up" style={{ animationDelay: `${i * 0.06}s` }}>
              <summary className="font-bold text-white list-none flex items-center justify-between">
                <span className="group-hover:text-[#00e6e6] transition-colors duration-300 pr-4">{faq.q}</span>
                <span className="text-white/15 group-open:rotate-180 transition-transform duration-300 flex-shrink-0 text-lg">▾</span>
              </summary>
              <p className="text-white/45 text-sm mt-4 leading-relaxed border-t border-white/5 pt-4">{faq.a}</p>
            </details>
          ))}
        </div>
      </section>
      ...
      <section className="text-center py-8 animate-slide-up">
        <div className="game-card !p-10 max-w-xl mx-auto" style={{ background: 'linear-gradient(135deg, rgba(0,230,230,0.06), rgba(0,153,255,0.04))' }}>
          ...
          <p className="text-white/40 mb-8">Try Web VR free in 60 seconds, or get a headset for the full experience.</p>
...
function VRMethodCard({ emoji, title, subtitle, description, steps, tips, ctaText, ctaLink, color, delay }: {
  ...
  return (
    ...
    >
      ...
      <h3 className="font-black text-white text-xl mb-1">{title}</h3>
        ...
        <div className="space-y-2">
          {steps.map((step, i) => (
            <div key={i} className="flex items-start gap-3 text-sm group/step">
              ...
              <span className="text-white/50 group-hover/step:text-white/70 transition-colors">{step}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="mb-6 p-4 rounded-xl border border-white/[0.04]" style={{ backgroundColor: `${color}05` }}>
        <h4 className="text-xs font-black tracking-[0.1em] mb-2 text-white/40">💡 TIPS</h4>
      ...
      <Link
        to={ctaLink}
        className="btn-elite inline-block text-sm text-white"
...
function QuickStep({ num, title, desc, icon }: { num: string; title: string; desc: string; icon: string }) {
  return (
    <div className="text-center group">
      ...
      <div className="text-3xl font-black text-[#00ff80] mb-1">{num}</div>
...
function Row({ label, v1, v2, v3, highlight }: { label: string; v1: string; v2: string; v3: string; highlight?: number }) {
  return (
    <tr className="border-b border-white/[0.03] hover:bg-white/[0.015] transition-colors duration-200">
      <td className="py-3 px-4 text-white/40 font-semibold text-xs">{label}</td>
      <td className={`py-3 px-4 text-xs ${highlight === 0 ? 'text-[#0099ff] font-bold' : 'text-white/60'}`}>{v1}</td>
      <td className={`py-3 px-4 text-xs ${highlight === 1 ? 'text-[#9933ff] font-bold' : 'text-white/60'}`}>{v2}</td>
      <td className={`py-3 px-4 text-xs ${highlight === 2 ? 'text-[#00ff80] font-bold' : 'text-white/60'}`}>{v3}</td>
    </tr>
  );
}
